...
return this.message.channel.send(`Aktif olarak oynadıgın bir oyun var zaten.`).sil(15);
} else if (midDuel.has(member.id)) {
return this.message.channel.send(`<@${member.id}> şu anda bir düelloda`).sil(15);
}
...
.setDescription(`**${this.message.member.displayName}** Sıra sende.`)
.setColor('Random');
} else {
Embed = new Discord.EmbedBuilder()
.setTitle(`${this.message.member.displayName} *vs* ${this.opponent.user.displayName}`)
.setDescription(`**${this.opponent.user.displayName}** Sıra sende.`)
.setColor('Random');
}
...
this.message.channel.send({content: `Tebrikler! ${gameData[player].member} Sen kazandın.`});
gameCollector.stop();
midDuel.delete(author);
midDuel.delete(member.id);
} else if (a1 !== "⬜" && a2 !== "⬜" && a3 !== "⬜" && b1 !== "⬜" && b2 !== "⬜" && b3 !== "⬜" && c1 !== "⬜" && c2 !== "⬜" && c3 !== "⬜") {
this.message.channel.send({content: `Oyun berabere bitti`});
gameCollector.stop();
midDuel.delete(author);
midDuel.delete(member.id);
}
...
player = (player + 1) % 2;
if (player == 0) {
Embed = new Discord.EmbedBuilder().setDescription(`${this.message.member.displayName} *vs* ${this.opponent.user.displayName}`).setColor('Random');
} else {
Embed = new Discord.EmbedBuilder().setDescription(`${this.message.member.displayName} *vs* ${this.opponent.user.displayName}`).setColor('Random');
}
...
if (player == 0) {
Embed = new Discord.EmbedBuilder()
.setDescription(`${authorName} VS ${this.opponent.user.displayName}`).setColor('Random');
} else {
Embed = new Discord.EmbedBuilder().setDescription(`${authorName} VS ${this.opponent.user.displayName}`).setColor('Random');
}
